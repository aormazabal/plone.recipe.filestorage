...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        if self.zeo_part is not None:
            if self.zeo_part in self.buildout:
                ...
                raise UserError('[plone.recipe.filestorage] "%s" '
            ...
            for part_name in active_parts:
                ...
                if 'recipe' not in part:
                    ...
                    if self.zeo_part is not None:
                        raise UserError('[plone.recipe.filestorage] "%s" part found multiple zeoserver parts; '
        ...
        if self.backup_part is not None:
            if not self.backup_part in self.buildout:
                raise UserError('[plone.recipe.filestorage] "%s" part specifies '
    ...
    def _validate_part_order(self):
        ...
        for part_name in self.buildout['buildout']['parts'].split():
            ...
            if part_name in target_parts:
                if len(injector_parts) > 0:
                    raise UserError(
                        '[plone.recipe.filestorage] The "%s" part must be listed '
        ...
        if len(target_parts) > 0:
            raise UserError(
                '[plone.recipe.filestorage] The "%s" part expected but failed '
